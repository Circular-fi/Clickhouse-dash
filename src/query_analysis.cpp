...
std::vector<QueryLogAnalysisRow> load_query_log_rows(
  ...
  const std::string query =
      ...
      "toString(event_time_microseconds), toUInt64(query_duration_ms), "
      ...
      "FROM system.query_log WHERE type != 'QueryStart' AND " + predicate + " "
  ...
  client.Select(query, [&](const clickhouse::Block& block) {
    for (size_t row = 0; row < block.GetRowCount(); ++row) {
      ...
      value.duration_ms = block_u64(block, 5, row);
      value.read_rows = block_u64(block, 6, row);
      value.read_bytes = block_u64(block, 7, row);
      value.written_rows = block_u64(block, 8, row);
      value.written_bytes = block_u64(block, 9, row);
      value.result_rows = block_u64(block, 10, row);
      value.result_bytes = block_u64(block, 11, row);
      value.memory_usage = block_i64(block, 12, row);
      value.peak_threads_usage = block_u64(block, 13, row);
      value.user_time_us = block_u64(block, 14, row);
      value.system_time_us = block_u64(block, 15, row);
      value.databases = split_unit_separator(block_string(block, 16, row));
      value.tables = split_unit_separator(block_string(block, 17, row));
      value.projections = split_unit_separator(block_string(block, 18, row));
      value.exception_code = block_i32(block, 19, row);
      value.exception = block_string(block, 20, row);
      value.query = block_string(block, 21, row);
      value.is_initial_query = block_u64(block, 22, row) != 0;
...
std::vector<ProcessorAnalysisRow> load_processors(
  ...
  const std::string query =
      ...
      "FROM system.processors_profile_log WHERE query_id IN " + id_array + " "
      "ORDER BY query_id, plan_step, plan_group, id LIMIT 10000";
...
std::vector<OpenTelemetrySpanAnalysisRow> load_otel_spans(
  ...
  const std::string query =
      ...
      "toString(operation_name), toUInt64(start_time_us), toUInt64(finish_time_us), "
      "toString(attribute['clickhouse.query_id']), toString(attribute['clickhouse.thread_id']), "
      "toString(attribute['thread_number']) "
      "FROM system.opentelemetry_span_log WHERE trace_id IN ("
      "SELECT DISTINCT trace_id FROM system.opentelemetry_span_log "
      "WHERE attribute['clickhouse.query_id'] IN " + id_array + ") "
      "ORDER BY start_time_us, finish_time_us, span_id LIMIT 10001";
  ...
  if (truncated) *truncated = rows.size() > kMaxTraceSpans;
  if (rows.size() > kMaxTraceSpans) rows.resize(kMaxTraceSpans);
  return rows;
}
...
std::vector<QueryViewAnalysisRow> load_views(
  ...
  const std::string query =
      ...
      "FROM system.query_views_log WHERE initial_query_id IN " + id_array + " "
...
QueryAnalysisResult collect_query_analysis(
  ...
  do {
    try {
      result.query_log = load_query_log_rows(*client, ids, false);
  ...
  if (result.query_log_available) {
    try {
      result.distributed_children = load_query_log_rows(*client, ids, true);
  ...
  try {
    result.processors = load_processors(*client, ids);
  ...
  if (record.run_mode == QueryRunMode::Profiling) {
    ...
    do {
      ...
      try {
        ...
        auto spans = load_otel_spans(*client, ids, &truncated);
  ...
  try {
    result.views = load_views(*client, ids);
