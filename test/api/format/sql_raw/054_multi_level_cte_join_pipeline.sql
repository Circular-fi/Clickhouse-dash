...
INNER JOIN grouped_averages AS grouped_averages ON metrics_src.entity_group = grouped_averages.entity_group
WHERE (metrics_src.total_metric_value > grouped_averages.avg_total_metric_value) AND (metrics_src.entity_key IN (
    SELECT entity_key
    FROM anon.metrics_store
    WHERE (event_date >= range_start) AND has(tag_values, 'priority')
))
ORDER BY
    metric_delta DESC,
